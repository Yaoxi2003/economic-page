export default function Cart({checkout, number, handleCart, openCart, totalAmount, handleRemove}) {
    ...
    const cartBtn = (
        <button onClick={handleCart} className="relative">
            <img src="/images/icon-cart.svg" alt="A shopping cart icon" />
            {checkout && <p className="hint">{number}</p>}
        </button>
    )
    ...
    const finalPayment = (
        <article>
            <div className="flex justify-between items-center gap-5">
                <img src="/images/image-product-1-thumbnail.jpg" alt="The final checkout product image" className="w-[50px] h-[50px] rounded-md" />
                <div>
                    <p>Fall Limited Edition Sneakers</p>
                    <p>{`$125.00 x ${number}`}   <span className="font-bold text-grey950">{`$${totalAmount}`}</span></p>
                </div>
                <button onClick={handleRemove}>
                    <img src="/images/icon-delete.svg" alt="A trash bin icon" />
                </button>
    ...
    return (
        <>
           ...
           {openCart && 
            <div className="absolute z-10 top-20 left-1/2 -translate-x-1/2 sm:translate-0 sm:top-35 lg:left-2/3 lg:-translate-x-1/10 w-[360px] h-[256px] 
